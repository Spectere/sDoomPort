	...
	LIST_ITERATE(th, &thinkers) {
		if(th->action.acp1 == (actionf_p1)P_MobjThinker) {
			*save_p++ = tc_mobj;
			PADSAVEP();
			mobj = (mobj_t *)save_p;
			memcpy(mobj, th->object, sizeof(*mobj));
			save_p += sizeof(*mobj);
...
void P_UnArchiveThinkers(void) {
	...
	while(1) {
		...
		switch(tclass) {
			...
			case tc_mobj:
				PADSAVEP();
				mobj = X_Malloc(sizeof(*mobj), XTag_Level);
...
void P_ArchiveSpecials(void) {
	...
	// save off the current thinkers
	LIST_ITERATE(th, &thinkers) {
		if(th->action.acv == (actionf_v)NULL) {
			...
			if(i < MAXCEILINGS) {
				*save_p++ = tc_ceiling;
				...
				memcpy(ceiling, th->object, sizeof(*ceiling));
		...
		if(th->action.acp1 == (actionf_p1)T_MoveCeiling) {
			*save_p++ = tc_ceiling;
			PADSAVEP();
			ceiling = (ceiling_t *)save_p;
			memcpy(ceiling, th->object, sizeof(*ceiling));
		...
		if(th->action.acp1 == (actionf_p1)T_VerticalDoor) {
			*save_p++ = tc_door;
			PADSAVEP();
			door = (vldoor_t *)save_p;
			memcpy(door, th->object, sizeof(*door));
			save_p += sizeof(*door);
			door->sector = (sector_t *)(door->sector - sectors);
			continue;
		}

		if(th->action.acp1 == (actionf_p1)T_MoveFloor) {
			*save_p++ = tc_floor;
			PADSAVEP();
			floor = (floormove_t *)save_p;
			memcpy(floor, th->object, sizeof(*floor));
			save_p += sizeof(*floor);
			floor->sector = (sector_t *)(floor->sector - sectors);
			continue;
		}

		if(th->action.acp1 == (actionf_p1)T_PlatRaise) {
			*save_p++ = tc_plat;
			PADSAVEP();
			plat = (plat_t *)save_p;
			memcpy(plat, th->object, sizeof(*plat));
			save_p += sizeof(*plat);
			plat->sector = (sector_t *)(plat->sector - sectors);
			continue;
		}

		if(th->action.acp1 == (actionf_p1)T_LightFlash) {
			*save_p++ = tc_flash;
			PADSAVEP();
			flash = (lightflash_t *)save_p;
			memcpy(flash, th->object, sizeof(*flash));
			save_p += sizeof(*flash);
			flash->sector = (sector_t *)(flash->sector - sectors);
			continue;
		}

		if(th->action.acp1 == (actionf_p1)T_StrobeFlash) {
			*save_p++ = tc_strobe;
			PADSAVEP();
			strobe = (strobe_t *)save_p;
			memcpy(strobe, th->object, sizeof(*strobe));
			save_p += sizeof(*strobe);
			strobe->sector = (sector_t *)(strobe->sector - sectors);
			continue;
		}

		if(th->action.acp1 == (actionf_p1)T_Glow) {
			*save_p++ = tc_glow;
			PADSAVEP();
			glow = (glow_t *)save_p;
			memcpy(glow, th->object, sizeof(*glow));
			save_p += sizeof(*glow);
...
void P_UnArchiveSpecials(void) {
	...
	while(1) {
		...
		switch(tclass) {
			...
			case tc_ceiling:
				PADSAVEP();
				ceiling = X_Malloc(sizeof(*ceiling), XTag_Level);
			...
			case tc_door:
				PADSAVEP();
				door = X_Malloc(sizeof(*door), XTag_Level);
			...
			case tc_floor:
				PADSAVEP();
				floor = X_Malloc(sizeof(*floor), XTag_Level);
			...
			case tc_flash:
				PADSAVEP();
				flash = X_Malloc(sizeof(*flash), XTag_Level);
			...
			case tc_strobe:
				PADSAVEP();
				strobe = X_Malloc(sizeof(*strobe), XTag_Level);
			...
			case tc_glow:
				PADSAVEP();
				glow = X_Malloc(sizeof(*glow), XTag_Level);
